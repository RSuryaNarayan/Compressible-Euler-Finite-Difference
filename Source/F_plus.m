function fp = F_plus(u, v, p, rho, xi_x, xi_y,J)
    ...
    L_p = zeros(4,4);
    L_p(1,1) = r_xi*0.5*(u_d-a+sqrt((u_d-a)^2+e^2));
    L_p(2,2) = r_xi*0.5*(u_d+sqrt(u_d^2+e^2));
    L_p(3,3) = r_xi*0.5*(u_d+a+sqrt((u_d+a)^2+e^2));
    L_p(4,4) = r_xi*0.5*(u_d+sqrt(u_d^2+e^2));
